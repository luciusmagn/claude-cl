...
This library provides a Common Lisp interface to Anthropic's Claude API and compatible Messages API providers. It allows you to integrate Claude-compatible capabilities into your Common Lisp applications.
...
(defun ask-openrouter (prompt)
  (let* ((message (make-message +role-user+
         ...
         (request (make-ai-request
                   ...
                   :api-url "https://openrouter.ai/api/v1/messages"
                   :auth-header-name "Authorization"
                   :auth-token-prefix "Bearer "
                   :include-anthropic-version-header nil
                   :anthropic-beta nil
                   :extra-headers '(("HTTP-Referer" . "https://your-app.example")
                                    ("X-Title" . "Your App")))))
...
*** Request Constructor
- ~(make-ai-request &key model messages max-tokens metadata stream system temperature top-k top-p api-key (anthropic-version "2023-06-01") tools tool-choice (api-url "https://api.anthropic.com/v1/messages") (auth-header-name "x-api-key") auth-token-prefix (extra-headers nil) (include-anthropic-version-header t) (anthropic-beta "max-tokens-3-5-sonnet-2024-07-15"))~ - Create an API request to Claude or a compatible endpoint
